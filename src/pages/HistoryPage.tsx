      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-3xl md:text-4xl font-bold text-white tracking-tight mb-1">
            Medication <span className="text-gradient text-gradient-blue">Check Log</span>
          </h2>
          <p className="text-white/40 font-medium">Historical safety checks and clinical advisory resolution.</p>
        </div>
      ...
      <div className="glass-card overflow-hidden">
        ...
        <div className="p-4 border-b border-white/6 flex gap-3" style={{ background: 'rgba(255,255,255,0.02)' }}>
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Search history log..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        {isLoading ? (
          <div className="py-20 flex flex-col items-center gap-4">
            ...
            <p className="text-white/25 text-xs font-mono uppercase tracking-widest">Loading safety log...</p>
          </div>
        ) : filtered.length === 0 ? (
          <div className="py-24 text-center flex flex-col items-center">
            ...
            <h3 className="text-lg font-bold text-white mb-2">{search ? 'No Results Found' : 'History Empty'}</h3>
            <p className="text-white/25 font-medium text-sm max-w-xs mx-auto">
              {search ? 'Try a different search term.' : 'No safety checks logged yet.'}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/5" style={{ background: 'rgba(0,0,0,0.15)' }}>
                  {['Timestamp / ID', 'Checked Medications', 'Severity Level', 'Alerts Found', ''].map(h => (
                  ...
                  return (
                    <tr key={log.id} className="border-b border-white/4 hover:bg-white/3 transition-colors group cursor-pointer">
                      ...
                      <td className={cn('py-4 px-5 text-sm font-bold', anomalyColor)}>
                        {log.issueCount > 0 ? `${log.issueCount} ALERTS` : '0 ALERTS'}
                      </td>
                      <td className="py-4 px-5 text-right">
                        <div className="w-8 h-8 ml-auto rounded-xl glass-card !rounded-xl !p-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:!bg-teal-500/20 hover:border-teal-400/30 text-white/30 hover:text-teal-400">
